...
- `lazyWithRetry` utility: auto-reloads the page once when a lazy-loaded chunk fails to fetch (stale cache after deploy), preventing the "Failed to fetch dynamically imported module" error.
...
- Translated the entire web UI from English to Bahasa Indonesia: `index.html` `lang="id"` and every user-facing string across pages, components, modals, toasts, placeholders, aria-labels, and legal content (Privacy Policy, Terms of Service). Technical terms (OAuth, S3, API, Drive, Workspace, Service Account, email, URL, Token, Key) and role identifiers (Owner, Editor, Viewer, Manager, super_admin, member) kept universal — they are system/proper nouns, not translatable UI. Glossary: Sign in/out → Masuk/Keluar, Dashboard → Dashboard, My Drive → Drive Saya, Starred → Berbintang, Trash → Sampah, Recent → Terbaru, Connected drives → Drive terhubung, Total storage → Total penyimpanan, By type → Per tipe, Refresh → Segarkan, View all → Lihat semua, Settings → Pengaturan, Users → Pengguna, Search → Cari, Password → Kata sandi. No i18n library introduced — direct string replacement (YAGNI; single locale, no runtime switching needed). Vitest assertions updated to match the Indonesian UI.
- Home dashboard "Editorial Hero" refresh: storage hero gains a Recharts `RadialBarChart` ring + cobalt gradient wash (replacing bare `%` + `QuotaBar`), quick-access becomes a vertical list (was 2×2 tiles), connected-drive cards get a drive-color left accent stripe + larger avatar + Primary pill, donut enlarged, Recent files switched to compact custom rows (file-type icon + drive dot + relative time) instead of `FileGrid` — Home is a glance dashboard, not a file manager.
- Landing page (`/home`) "Product Showcase" refresh: asymmetric 5/7 hero (left text + CTAs, right pure-CSS dashboard mockup echoing the redesigned Home bento), stats strip ($0 cost · Multi-drive · S3-compatible), features become alternating left/right rows (was uniform 3-col card grid), added final CTA section. Inline SVG ring instead of Recharts on landing to keep public-page LCP light.
...
- Upload modal no longer disappears mid-upload: `FilesPage` passed an `onSuccess` that force-closed the modal the instant `startUpload` resolved, bypassing `UploadModal.handleClose` so `clearQueue` never ran. The next "Unggah" click reopened the modal still carrying the stale `done` queue (showing the old "Selesai" view instead of the file picker). `onSuccess` now only refreshes the file list; the modal stays open to show upload progress and is closed via "Selesai"/"Batal", which clears the queue.
- File grid view cards (folders + files) now use `bg-background` (the canvas token `#faf9f5`) with `border-stone-200` so they stand out from the cream card container (`#efe9de`, 1.15:1 contrast) instead of blending in. Replaces the earlier `bg-white` change so the cards stay on the design-system color palette.
