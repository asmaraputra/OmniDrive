    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md p-0 gap-0 rounded-2xl overflow-hidden flex flex-col max-h-[80vh]">
        <div className="flex items-center px-6 py-4 border-b border-stone-100 shrink-0">
          <DialogTitle className="text-xl font-semibold text-stone-800">Tambah ke Workspace</DialogTitle>
        </div>
        ...
        <div className="px-6 py-4 border-t flex justify-end gap-3 shrink-0">
          <button onClick={onClose} className="px-4 py-2 font-medium text-stone-700 bg-stone-100 rounded-lg hover:bg-stone-200 transition-colors">Batal</button>
          <button onClick={handleAdd} disabled={!selectedId} className="px-4 py-2 font-medium text-white bg-primary rounded-lg hover:bg-primary/90 disabled:opacity-50 transition-colors">Tambah</button>
        </div>
      </DialogContent>
    </Dialog>
  );
