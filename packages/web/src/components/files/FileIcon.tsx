...
export function FileIcon({ mimeType, className }: FileIconProps) {
  const getIconInfo = () => {
    ...
    if (mimeType.startsWith('image/')) return { Icon: ImageIcon, color: 'text-primary' };
    ...
    if (mimeType.includes('document') || mimeType.includes('word') || mimeType.includes('text')) return { Icon: FileText, color: 'text-primary' };
