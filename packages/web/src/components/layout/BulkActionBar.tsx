...
export const BulkActionBar: React.FC<BulkActionBarProps> = ({ onActionComplete, onMoveRequested, onWorkspaceRequested, onMoveDriveRequested }) => {
  ...
  return (
    <div className="fixed bottom-4 left-2 right-2 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 z-50 flex flex-wrap items-center gap-2 bg-white/80 backdrop-blur-md border border-gray-200 text-gray-800 rounded-2xl sm:rounded-full shadow-2xl px-4 py-3 animate-in fade-in-0 slide-in-from-bottom-5 duration-300">
      <div className="flex items-center gap-3 sm:border-r sm:border-gray-200 sm:pr-4">
        <button onClick={clearSelection} disabled={isProcessing} className="p-2 hover:bg-gray-100 text-gray-500 rounded-full transition-colors" aria-label="Clear selection">
          <X size={18} />
        </button>
      ...
      <div className="flex flex-wrap items-center gap-1 sm:gap-2 sm:pl-2">
        <button onClick={handleDelete} disabled={isProcessing} className="flex items-center gap-2 px-3 py-2 hover:bg-red-50 text-gray-600 hover:text-red-600 rounded-full transition-colors text-sm font-medium" title="Delete selected items">
          <Trash2 size={16} /> <span>Delete</span>
        </button>
        <button onClick={onMoveRequested} disabled={isProcessing} className="flex items-center gap-2 px-3 py-2 hover:bg-gray-100 text-gray-600 rounded-full transition-colors text-sm font-medium" title="Move selected items">
          <Folder size={16} /> <span>Move</span>
        </button>
        <button
          onClick={onMoveDriveRequested}
          disabled={isProcessing || !allFiles}
          className={`flex items-center gap-2 px-3 py-2 rounded-full transition-colors text-sm font-medium ${!allFiles ? 'opacity-50 cursor-not-allowed text-gray-400' : 'hover:bg-gray-100 text-gray-600'}`}
